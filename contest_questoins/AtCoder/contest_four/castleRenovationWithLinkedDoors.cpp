...
vector<pair<int, int>> get_path(pair<int, int> start, pair<int, int> target, 
                                bool blocked[N][N][2], const vector<string>& grid) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (blocked[r_min][c_min][d_type]) continue;
    ...
    if (!visited[target.first][target.second]) return path; // Target Unreachable
...
vector<vector<int>> get_all_reachable_distances(pair<int, int> start, bool blocked[N][N][2], const vector<string>& grid) {
    queue<pair<int, int>> q;
...
int main() {
    ...
    if (!(cin >> n_input >> m >> k_input)) return 0;
    ...
    // Tracks doors we have placed so we don't route through them during generation
    ...
    for (int k = 0; k < k_input; ++k) {
        ...
        // If we can't reach the target, our maze is as complex as this logic can make it.
        if (path.size() < 2) break; 

        // 2. Pick a choke point (edge) to block with a CLOSED door.
        // We pick roughly the midpoint of the path to force the hero to walk deep into the maze.
