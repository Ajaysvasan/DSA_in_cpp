...
struct Details {
  ...
  bool operator==(const Details &details) { return this->name == details.name; }
};
...
struct Collect {
  ...
  void operator-=(const Details &details) { this->get_details.remove(details); }
};
...
operator*(const std::vector<std::vector<int>> &arr1,
          const std::vector<std::vector<int>> &arr2) {
  if (arr1[0].size() != arr2.size())
    return {{}};
  std::vector<std::vector<int>> result(arr1[0].size(),
...
main() {
  ...
  std::vector<std::vector<int>> matrix_three = {
      {1, 2, 3}, {4, 5, 6}, {7, 8, 9}};
