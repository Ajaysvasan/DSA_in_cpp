...
private:
  ...
  bool searchMax(int target, int curr_index) {
    if (heap.empty() || curr_index >= heap.size())
      return false;

    if (heap[curr_index] == target)
      return true;
    return searchMax(target, 2 * curr_index + 1) ||
           searchMax(target, 2 * curr_index + 2);
  }
...
public:
  ...
  bool search(int target) { return searchMax(target, 0); }
...
private:
  ...
  void heapifydown(int i) {
    ...
    while (2 * i + 1 < n) {
      ...
      int left = 2 * i + 1;
      int right = 2 * i + 2;
      if (left < n && heap[left] < heap[min_index])
        min_index = left;
      if (right < n && heap[right] < heap[min_index])
        min_index = right;
      if (min_index == i)
        break;
  ...
  bool searchMin(int target, int curr_index) {
    if (heap.empty() || heap.size() <= curr_index)
      return false;
    if (heap[curr_index] > target) {
      return false;
    }
    if (heap[curr_index] == target)
      return true;
    return searchMin(target, 2 * curr_index + 1) ||
           searchMin(target, 2 * curr_index + 2);
  }
...
public:
  ...
  bool search(int target) { return searchMin(target, 0); }
